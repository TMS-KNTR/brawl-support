...
| **Webhook（ウェブフック）** | 「〇〇が起きたら、このURLに通知して」という仕組み。UnivaPay が「決済が完了したよ」とあなたのサーバーに知らせるために使います。 |
...
## やること②：UnivaPay の Webhook を設定する（本番で決済する場合）

### なぜ必要？

- お客様が UnivaPay で支払いを完了すると、**UnivaPay が「決済完了」をあなたのサーバーに通知**する必要があります。
- この通知（Webhook）を受け取ることで、注文の状態が「決済待ち（pending_payment）」→「支払済（paid）」に更新され、依頼の表示や後の処理が正しく動きます。

### 手順の流れ（概要）

1. **UnivaPay の管理画面（merchant.univapay.com）を開く**
   - ブラウザで [https://merchant.univapay.com](https://merchant.univapay.com) にアクセスし、ログインします。

2. **Webhook の「エンドポイント」を追加する**
   - 左メニューで **設定** → **Webhook**（または **ウェブフック**）を開きます。
   - **新規追加** をクリックします。
   - **URL** に、Supabase が用意している URL を入れます。形は次のとおりです。
     ```
     https://あなたのプロジェクトID.supabase.co/functions/v1/univapay-webhook
     ```
     「あなたのプロジェクトID」は、Supabase の **Settings** → **API** の **Project URL** の `https://xxxxx.supabase.co` の `xxxxx` の部分です。
   - **トリガー** で、少なくとも **charge_finished** にチェックを入れます。
   - **認証方式** で **auth_token**（Bearer トークン）を選び、自分で決めた長いランダム文字列を入力します（例：40文字以上の英数字）。この値はあとで Supabase 側にも同じものを設定するので、メモ帳にコピーしておきます。
   - **保存** で確定します。

3. **Supabase に秘密の値を登録する**
   - Supabase ダッシュボード → **Project Settings** → **Edge Functions**（または **Secrets**）を開きます。
   - 次の4つを **1つずつ** 追加します。

   | 名前（Name） | 値（Value） |
   |---|---|
   | **UNIVAPAY_APP_TOKEN** | UnivaPay 管理画面の **アプリトークン**（フロントエンドのウィジェットで使用） |
   | **UNIVAPAY_SECRET** | UnivaPay 管理画面の **シークレット**（返金APIなどサーバー側API呼び出しで使用） |
   | **UNIVAPAY_STORE_ID** | UnivaPay 管理画面の **ストアID** |
   | **UNIVAPAY_WEBHOOK_AUTH_TOKEN** | 手順2で決めた auth_token と**同じ値** |

   - これで、本番の `univapay-webhook` が「本当に UnivaPay から来た通知か」を検証できるようになります。

### テスト環境だけ使う場合

- UnivaPay にはテストモードがあり、テスト用のアプリトークン・シークレットが別途発行されます。テスト用と本番用は混在させないように注意し、同じ手順でテスト用の値を Supabase Secrets に設定してください。
...
- **決済後に注文が「支払済」にならない**
  - ① UnivaPay 管理画面の Webhook 設定で **charge_finished** トリガーが有効になっているか。
  - ② Webhook の URL が `https://xxxxx.supabase.co/functions/v1/univapay-webhook` になっているか。
  - ③ Supabase に **UNIVAPAY_APP_TOKEN** / **UNIVAPAY_SECRET** / **UNIVAPAY_STORE_ID** / **UNIVAPAY_WEBHOOK_AUTH_TOKEN** をすべて設定したか。
  - ④ UnivaPay 管理画面の **Webhook 配信履歴** で、そのエンドポイントの最新配信が成功（200）になっているか。
  - ⑤ Supabase の **Edge Functions → Logs** で `univapay-webhook` のログを確認し、`Charge finished` ログが出ているか。
...
| やりたいこと | やること |
|--------------|----------|
| 決済完了した依頼を従業員に表示したい | ① **list-available-orders** のデプロイ（推奨）。② 本番で決済するなら **UnivaPay Webhook** の設定。 |
